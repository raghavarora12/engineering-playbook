...
flowchart LR
    subgraph SEP["Separate planes + pipeline"]
        ...
        p1 --> a1["OLAP / lakehouse"]
    end
    subgraph CON["Converged (HTAP)"]
        direction LR
        o2["Transactional +<br/>analytical on one store"]
    end
```
...
| **Converged (HTAP / streaming lakehouse)** | Seconds / live | One system, but a less mature, more expensive one | Weak: mixed workload contends on one engine; analytical load shares OLTP's blast radius | Decisions must be made on data fresher than the pipeline delivers, *and* that freshness is worth more than the isolation lost. |

## Decision

[AUTHOR: which you chose, for which system.] The test that should decide it: convergence wins only
when **the decision has to be made on data fresher than your pipeline latency, and the value of
that freshness exceeds the isolation you give up.** Live fraud/risk decisioning on in-flight
transactions is the canonical case that clears that bar. A daily revenue dashboard is not — paying
HTAP's contention and operational cost to make it "live" is complexity with no buyer.

Absent that live-latency pressure, keep the planes separate: the pipeline lag is real but cheap,
and the isolation is worth keeping.
...
- **Contention and blast radius.** Analytical queries and transactions share one engine; a heavy
  scan can now hurt your transactional path. You've coupled two workloads that separation kept
  apart.
- **Operational maturity.** HTAP / streaming-lakehouse tooling is younger and pricier than the
  battle-tested OLTP and OLAP stacks it replaces.
...
- **Keeping them separate** costs you freshness and a pipeline to run — the right price to pay
  unless live latency is genuinely load-bearing. [AUTHOR: the freshness SLA that actually mattered
  for your case.]
